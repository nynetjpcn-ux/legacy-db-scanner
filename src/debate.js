...
async function askBuzzAnalyst(openai, theme, transcript) {
  const res = await openai.chat.completions.create({
    model: 'gpt-4o-mini',
    messages: [
      { role: 'system', content: BUZZ_ANALYST_PERSONA },
      {
        role: 'user',
        content:
          `テーマ: 「${theme}」\n\n` +
 ...
 */
export async function runDebate({ openaiApiKey, anthropicApiKey, theme }) {
  ...
  for (let round = 1; round <= ROUNDS; round++) {
    const buzzText = await askBuzzAnalyst(openai, theme, transcript);
